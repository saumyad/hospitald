...
ans = ''
def extract(filen,bed1,opr1,i1):
	filename = open(filen)
...
#	print ans		### the generated coefficients
	final,ans = calculate_wait(opr1,bed1,i1,ans)
	print "filemein",final,ans
	return final,ans


def calculate_wait(no_of_or, beds, h_eff,ans):
	wait_time = (ans[0]*no_of_or + ans[1]*beds + ans[2]*h_eff + ans[3]*beds*beds + ans[4]*no_of_or*no_of_or + ans[5] *h_eff*h_eff + ans[6] * beds * no_of_or + ans[7] * beds * h_eff + ans[8] * no_of_or * h_eff )/(24*360)
	final =  (1/wait_time)*100
	return final,ans


def main():
	extract('data2.txt',100,5,600)
	extract('data2.txt',100,5,1200)
	extract('data2.txt',100,20,600)
	pass
